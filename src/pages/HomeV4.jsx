...
/**
 * HomeV4 - Version avec vidéos plus hautes (aspect 4/5)
 * Ordre des services: Lavage, Polish, Cire, Céramique, Autres (comme homepage Wix)
 */
const HomeV4 = () => {
...
/**
 * ParallaxServicesV4 - Version avec vidéos hautes (4/5)
 * Ordre: Lavage, Polish, Cire, Céramique, Autres (comme homepage Wix)
 */
const ParallaxServicesV4 = () => {
  const services = [
    {
      title: 'Lavage',
      subtitle: 'Premium.',
...
/**
 * ServiceCardV4 - Carte avec vidéos hautes (aspect 4/5)
 */
const ServiceCardV4 = ({ service, index, isLast, totalCards }) => {
  ...
  return (
    <div
      ref={cardRef}
      className="sticky top-[72px] min-h-[calc(100vh-72px)] flex items-center"
      style={{ zIndex: index + 1 }}
    >
      {/* Background avec couleur alternée subtile */}
      <div className={`absolute inset-0 ${index % 2 === 0 ? 'bg-[#f8f7f4]' : 'bg-white'}`} />

      <div className="relative z-10 w-full max-w-[1600px] mx-auto px-6 lg:px-12 py-8 lg:py-12">
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center ${!isEven ? 'lg:flex-row-reverse' : ''}`}>
          ...
          {/* Video/Image - Plus haute avec aspect 4/5 */}
          <motion.div
            style={{ y: imageY }}
          ...
          >
            <div className="relative">
              {/* Ombre sous le media */}
              <div className="absolute -bottom-4 left-4 right-4 h-20 bg-dark-900/10 blur-2xl rounded-full" />

              {/* Container media avec bordure arrondie - aspect ratio HAUT 4/5 */}
              <div className="relative aspect-[4/5] rounded-2xl lg:rounded-3xl overflow-hidden shadow-2xl shadow-dark-900/20">
                {service.video ? (
                  <video
                    src={service.video}
                    autoPlay
